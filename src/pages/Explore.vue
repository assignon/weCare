            ...
            <div class="d-flex align-center justify-space-between mb-3">
              <div class="flex-grow-1 mr-4">
                ...
                <v-select
                  ...
                  :items="productStore.skinTypes"
                  item-title="name"
                  item-value="id"
                  label="Select skin types"
                  multiple
                  chips
                  variant="outlined"
                  density="compact"
              ...
              <div class="price-sort-buttons">
                <span class="text-subtitle-2 mb-2 d-block">Sort by Price:</span>
                <v-btn 
                  size="small"
                  variant="outlined"
                  color="primary"
                  @click="togglePriceSort"
                  class="text-none price-toggle-btn"
                >
...
// Computed property to handle local filtering for multiple skin types
const filteredProducts = computed(() => {
  let products = productStore.products
  
  // If multiple skin types are selected, filter locally
...
const togglePriceSort = async () => {
  ...
  isFiltering.value = true
  const hasMore = await productStore.fetchProducts({ ordering: sortOrder.value === 'asc' ? 'price' : '-price' })
  hasMoreProducts.value = hasMore
}
...
onMounted(async () => {
  try {
    // Initialize cart state and fetch categories and skin types
    cart.initCartState()

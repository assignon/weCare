  ...
  <div class="page-container pb-24">
    ...
    <div class="p-3 pt-4">
      ...
      <div v-if="loading" class="text-center py-12">
        ...
        <h3 class="text-sm font-semibold text-navy mb-1">{{ $t('profile.addresses_page.loading') }}</h3>
        <p class="text-xs text-grey-400">{{ $t('profile.addresses_page.loading_subtitle') }}</p>
      </div>
      ...
      <div v-else class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center justify-between mb-2">
              <div class="flex items-center space-x-2">
                <div class="w-6 h-6 bg-navy/10 rounded-xl flex items-center justify-center flex-shrink-0">
                  <MapPin class="w-3 h-3 text-navy" />
                </div>
                <div>
                  <h3 class="text-sm font-bold text-navy">{{ address.address_label || $t('profile.addresses_page.address') }}</h3>
                  <p class="text-xs text-grey-400">{{ $t('profile.addresses_page.delivery_location') }}</p>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-1.5">
                <span 
                  ...
                  class="px-2 py-0.5 bg-success-100 text-success-700 text-xs font-medium rounded-full flex items-center space-x-0.5"
                >
            ...
            <!-- Address Details -->
            <div class="card-flat p-2.5 mb-2">
              <div class="text-xs text-grey-500 space-y-0.5">
                <p class="font-medium text-navy">{{ address.address_line1 }}</p>
                <p v-if="address.address_line2" class="text-grey-400">{{ address.address_line2 }}</p>
                <p class="text-grey-400">
                  {{ address.city }}{{ address.state ? `, ${address.state}` : '' }} {{ address.postal_code }}
                </p>
                <p class="text-grey-400">{{ address.country }}</p>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between">
              <div>
                <button 
                  ...
                  :disabled="loading"
                  class="px-2.5 py-1.5 text-navy hover:text-navy/80 text-xs font-semibold bg-navy/5 hover:bg-navy/10 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
                >
              ...
              <div class="flex items-center space-x-1.5">
                <button 
                  ...
                  class="w-8 h-8 bg-grey-50 hover:bg-grey-100 rounded-xl flex items-center justify-center transition-all duration-200"
                >
                  <Edit3 class="w-3.5 h-3.5 text-grey-500" />
                </button>
                
                <button 
                  @click="confirmDeleteAddress(index)"
                  :disabled="address.is_default"
                  class="w-8 h-8 bg-error-50 hover:bg-error-100 rounded-xl flex items-center justify-center transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Trash2 class="w-3.5 h-3.5 text-error-600" />
                </button>
              </div>
            </div>
          </div>
        ...
        <div v-else class="text-center py-12">
          <div class="w-16 h-16 bg-grey-50 rounded-full flex items-center justify-center mx-auto mb-4">
            <MapPinOff class="w-8 h-8 text-grey-300" />
          </div>
          <h3 class="text-lg font-semibold text-navy mb-1">{{ $t('profile.addresses_page.no_addresses') }}</h3>
          <p class="text-sm text-grey-400 mb-4">{{ $t('profile.addresses_page.no_addresses_message') }}</p>
        </div>
        
        <!-- Add New Address Button -->
        <button 
          @click="addNewAddress"
          class="btn-cta flex items-center justify-center space-x-1.5"
        >
          <Plus class="w-4 h-4" />
    ...
    >
      <div class="bg-white rounded-3xl p-4 max-w-md w-full shadow-float max-h-[90vh] overflow-y-auto">
        <!-- Dialog Header -->
        <div class="flex items-center justify-between mb-3">
          ...
          <button 
            ...
            class="w-6 h-6 bg-grey-50 hover:bg-grey-100 rounded-xl flex items-center justify-center transition-all duration-200"
          >
            <X class="w-3.5 h-3.5 text-grey-500" />
          </button>
        </div>
        ...
        <form @submit.prevent="saveAddress" class="space-y-2.5">
          ...
          <div>
            <label class="block text-xs font-semibold text-navy mb-1">
              {{ $t('profile.addresses_page.address_label') }} <span class="text-error-600">{{ $t('profile.addresses_page.address_label_required') }}</span>
            </label>
            <input 
          ...
          <div>
            <label class="block text-xs font-semibold text-navy mb-1">
              {{ $t('profile.addresses_page.street_address') }} <span class="text-error-600">{{ $t('profile.addresses_page.street_address_required') }}</span>
            </label>
            <input 
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block text-xs font-semibold text-navy mb-1">
                {{ $t('profile.addresses_page.city') }} <span class="text-error-600">{{ $t('profile.addresses_page.city_required') }}</span>
              </label>
              <input 
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            <div>
              <label class="block text-xs font-semibold text-navy mb-1">
                {{ $t('profile.addresses_page.country') }} <span class="text-error-600">{{ $t('profile.addresses_page.country_required') }}</span>
              </label>
              <select 
          ...
          <div class="flex items-center space-x-2 pt-1.5">
            ...
            />
            <label for="default-address" class="text-xs text-grey-500">
              {{ $t('profile.addresses_page.set_as_default_address') }}
            </label>
          </div>

          <!-- Action Buttons -->
          <div class="flex space-x-2 pt-4 border-t border-grey-100">
            <button 
              type="button"
              @click="showAddressDialog = false"
    ...
    <div 
      ...
      class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div class="bg-white rounded-3xl p-5 max-w-md w-full shadow-float">
        <div class="flex items-center space-x-2 mb-4">
          <div class="w-10 h-10 bg-error-50 rounded-full flex items-center justify-center flex-shrink-0">
            <AlertTriangle class="w-5 h-5 text-error-600" />
          </div>
          <h2 class="text-base font-bold text-navy">{{ $t('profile.addresses_page.delete_address') }}</h2>
        </div>
        ...
        <div class="flex space-x-2">
          ...
          <button 
            @click="deleteAddress"
            :disabled="loading"
            class="flex-1 py-2.5 bg-error-600 text-white text-sm font-semibold rounded-2xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-1.5"
          >
            <Loader2 v-if="loading" class="w-3.5 h-3.5 animate-spin" />
    ...
    <div 
      ...
      class="fixed bottom-4 left-4 right-4 z-50 bg-success-600 text-white p-3 rounded-2xl shadow-float flex items-center space-x-2"
    >
    ...
    <div 
      ...
      class="fixed bottom-4 left-4 right-4 z-50 bg-error-600 text-white p-3 rounded-2xl shadow-float flex items-center space-x-2"
    >
